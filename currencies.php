...
<body>
    ...
    <form action="currencies.php" method="get">
        ...
        <fieldset>
            ...
            $qry_count = "SELECT word FROM words where category like 'currency'";
            ...
            for ($i = 1; $i <= $count; $i++) {
                ...
                $query = "SELECT word FROM words where category like 'currency' ORDER BY RAND() limit 1  ";
                $result = mysqli_query($conn, $query);
                $row = mysqli_fetch_assoc($result);

                // Shuffle the answer choices
                $choices = array($row['word']);
                $query = "SELECT word FROM words where category like 'currency' and word != '{$row['word']}' ORDER BY RAND() LIMIT 2";
                $result = mysqli_query($conn, $query);
